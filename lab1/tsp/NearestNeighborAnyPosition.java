...
public class NearestNeighborAnyPosition {
    ...
    /**
     * Finds a Hamiltonian path using a nearest neighbor approach, allowing insertion at any position in the path.
     *
     ...
     */
    public static List<Integer> nearestNeighborAnyPosition(double[][] distanceMatrix, int startNode) {
        ...
        for (int i = 1; i < n; i++) {
            ...
            for (int candidateNode = 0; candidateNode < n; candidateNode++) {
                if (!visited[candidateNode]) {
                    // Check each possible position in the current path
            ...
            // Add the best node at the best position
